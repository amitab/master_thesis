...
def weight_sim(model1, model2):
  ...
  for i, layer1 in enumerate(model1.layers):
    ...
    for j, layer2 in enumerate(model2.layers):
      ...
      info['w1']['sparisty<0.00001'] = 1.0 - (np.count_nonzero(a) / float(k))
      a[np.abs(a) < 0.0001] = 0
      info['w1']['sparisty<0.0001'] = 1.0 - (np.count_nonzero(a) / float(k))
      a[np.abs(a) < 0.001] = 0
      info['w1']['sparisty<0.001'] = 1.0 - (np.count_nonzero(a) / float(k))
      a[np.abs(a) < 0.01] = 0
      info['w1']['sparisty<0.01'] = 1.0 - (np.count_nonzero(a) / float(k))
      a[np.abs(a) < 0.1] = 0
      info['w1']['sparisty<0.1'] = 1.0 - (np.count_nonzero(a) / float(k))

      b[np.abs(b) < 0.00001] = 0
      info['w2']['sparisty<0.00001'] = 1.0 - (np.count_nonzero(b) / float(k))
      b[np.abs(b) < 0.0001] = 0
      info['w2']['sparisty<0.0001'] = 1.0 - (np.count_nonzero(b) / float(k))
      b[np.abs(b) < 0.001] = 0
      info['w2']['sparisty<0.001'] = 1.0 - (np.count_nonzero(b) / float(k))
      b[np.abs(b) < 0.01] = 0
      info['w2']['sparisty<0.01'] = 1.0 - (np.count_nonzero(b) / float(k))
      b[np.abs(b) < 0.1] = 0
